package com.mgr.kgu.VO;

import java.util.Date;

//과목 vo
public class SUB_VO {

	private int SUB_NUM;
	private int PROF_NUM;
	private String SUB_NAME;
	private String SUB_TIME;
	private int SUB_ROOM;
	private Date SUB_STADAY;
	private Date SUB_ENDDAY;
	private int SUB_MAX;
	private int SUB_MIN;
	private String SUB_STATE;
	
	public int getSUB_NUM() {
		return SUB_NUM;
	}
	public void setSUB_NUM(int sUB_NUM) {
		SUB_NUM = sUB_NUM;
	}
	public int getPROF_NUM() {
		return PROF_NUM;
	}
	public void setPROF_NUM(int pROF_NUM) {
		PROF_NUM = pROF_NUM;
	}
	public String getSUB_NAME() {
		return SUB_NAME;
	}
	public void setSUB_NAME(String sUB_NAME) {
		SUB_NAME = sUB_NAME;
	}
	public String getSUB_TIME() {
		return SUB_TIME;
	}
	public void setSUB_TIME(String sUB_TIME) {
		SUB_TIME = sUB_TIME;
	}
	public int getSUB_ROOM() {
		return SUB_ROOM;
	}
	public void setSUB_ROOM(int sUB_ROOM) {
		SUB_ROOM = sUB_ROOM;
	}
	public Date getSUB_STADAY() {
		return SUB_STADAY;
	}
	public void setSUB_STADAY(Date sUB_STADAY) {
		SUB_STADAY = sUB_STADAY;
	}
	public Date getSUB_ENDDAY() {
		return SUB_ENDDAY;
	}
	public void setSUB_ENDDAY(Date sUB_ENDDAY) {
		SUB_ENDDAY = sUB_ENDDAY;
	}
	public int getSUB_MAX() {
		return SUB_MAX;
	}
	public void setSUB_MAX(int sUB_MAX) {
		SUB_MAX = sUB_MAX;
	}
	public int getSUB_MIN() {
		return SUB_MIN;
	}
	public void setSUB_MIN(int sUB_MIN) {
		SUB_MIN = sUB_MIN;
	}
	public String getSUB_STATE() {
		return SUB_STATE;
	}
	public void setSUB_STATE(String sUB_STATE) {
		SUB_STATE = sUB_STATE;
	}

}
